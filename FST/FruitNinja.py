...
#Date: May 27, 2020
#Filename: FST3.py
#Description: The game "Fruit Ninja". The levels which get progressively harder. To pause, press "p". 
# How to play:
...
import os.path
from os import path
# This makes it not stutter for some reason, I found it online.
...
def border_gravity(fruit,x_pos,y_pos,x_velocity,y_velocity):
    ...
    if round(x_pos) < 1:
        # Set the x velocity equal to negative 0.9 times what it was, to simulate energy losses due to heat, sound, etc.
...
def hit_check(f_x,f_y,fruit_w,fruit_h,m_x,m_y):
    """
    This function calculates whether an inputted fruit has come into contact with the cursor.
...
while not done:
    ...
    to_be_del_slice = []    # Fills up with fruit slices that need to be deleted.                   
    if playing:        
        ...
        # If there are fewer fruit on screen than the maximum number allowed,
            ...
            if hit_check(round(x_pos),round(y_pos),fruit_w,fruit_h,m_x,m_y):
                if reset_count == 0:    
                    ...
                    if screen_fruit_list[i] == bomb:
                        ...
                        # Send the slice to be deleted (after this loop)
                        to_be_del.append(i)
    ...
    for s in range(slice_draw_loop_range):
        ...
        # Draw the current iterated slice on screen. Round the position because the positions are not whole integers, but pixels need to be.
        screen.blit(screen_slice_list[s],[round(sx_pos),round(sy_pos)])
        # If the slice has fallen past 600 on the y axis
        if sy_pos > 600:
            # Add the slice to be deleted, and escape the loop.
            to_be_del_slice.append(s)
            break
    ...
    for k in to_be_del:
        try:
            ...
            pass
        num_fruit -= 1
    # For every entry on the to be deleted slice list, delete the corresponding slice and its properties.
    for d in to_be_del_slice:
        try:
            slice_props.pop(d)
            screen_slice_list.pop(d)
        except:
            print("Delete error")
